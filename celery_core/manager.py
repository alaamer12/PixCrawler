...
class TaskInfo(BaseModel):
    """Enhanced information about a submitted task with Pydantic V2 validation."""
    ...
    kwargs: Dict[str, Any] = Field(
        ...
        examples=[{}, {"param1": "value1"}, {"max_images": 100}]
    )
    submitted_at: float = Field(
        default_factory=time.time,
        ge=0,
        description="Task submission timestamp",
        examples=[1640995200.0, 1640995260.5]
    )
    queue: Optional[str] = Field(
        default=None,
        max_length=50,
        pattern=r'^[a-zA-Z0-9_\-]*$',
        description="Queue name where task was submitted",
        examples=[None, "default", "high_priority", "background"]
    )
    ...
    @classmethod
    def validate_submission_time(cls, v: float) -> float:
        """Validate submission timestamp is reasonable."""
        current_time = time.time()
        if v > current_time + 60:  # Allow 1 minute future tolerance
            raise ValueError("submitted_at cannot be significantly in the future")
        if v < current_time - 86400 * 7:  # 7 days ago
            raise ValueError("submitted_at cannot be more than 7 days in the past")
        return v
