...
class TestLocalStorageProviderInit:
    ...
    def test_init_creates_directory_if_not_exists(self, temp_storage_dir: Path):
...
class TestLocalStorageProviderUpload:
    ...
    def test_upload_file_success(self, local_storage_provider: LocalStorageProvider, sample_text_file: Path):
        """Test successful file upload."""
        destination = "uploads/test.txt"
        local_storage_provider.upload(sample_text_file, destination)

        uploaded_file = local_storage_provider.base_directory / destination
        assert uploaded_file.exists()
        assert uploaded_file.read_text() == "Sample content for testing"

    def test_upload_creates_subdirectories(self, local_storage_provider: LocalStorageProvider, sample_text_file: Path):
    ...
    def test_upload_overwrites_existing_file(self, local_storage_provider: LocalStorageProvider, sample_text_file: Path):
        ...
        # Modify source file
        sample_text_file.write_text("Updated content")
    ...
    def test_upload_preserves_file_metadata(self, local_storage_provider: LocalStorageProvider, sample_text_file: Path):
    ...
    def test_upload_nonexistent_file_raises_error(self, local_storage_provider: LocalStorageProvider):
        """Test upload with nonexistent file raises FileNotFoundError."""
        with pytest.raises(FileNotFoundError, match="Source file not found"):
            local_storage_provider.upload("nonexistent.txt", "destination.txt")

    def test_upload_directory_raises_error(self, local_storage_provider: LocalStorageProvider, temp_storage_dir: Path):
        """Test upload with directory raises ValueError."""
        directory = temp_storage_dir / "test_dir"
        directory.mkdir()

        with pytest.raises(ValueError, match="Source path is not a file"):
            local_storage_provider.upload(directory, "destination.txt")

    def test_upload_with_path_object(self, local_storage_provider: LocalStorageProvider, sample_text_file: Path):
    ...
    def test_upload_image_file(self, local_storage_provider: LocalStorageProvider, sample_image_file: Path):
        """Test upload of binary image file."""
        destination = "images/test.png"
        local_storage_provider.upload(sample_image_file, destination)

        uploaded_file = local_storage_provider.base_directory / destination
        assert uploaded_file.exists()
        assert uploaded_file.read_bytes() == sample_image_file.read_bytes()


class TestLocalStorageProviderDownload:
    """Test LocalStorageProvider download operations."""

    def test_download_file_success(self, local_storage_provider: LocalStorageProvider, sample_text_file: Path, temp_storage_dir: Path):
        """Test successful file download."""
        # Upload file first
        storage_path = "test.txt"
        local_storage_provider.upload(sample_text_file, storage_path)

        # Download to different location
        download_path = temp_storage_dir / "downloaded.txt"
        local_storage_provider.download(storage_path, download_path)

        assert download_path.exists()
        assert download_path.read_text() == "Sample content for testing"

    def test_download_creates_destination_directory(self, local_storage_provider: LocalStorageProvider, sample_text_file: Path, temp_storage_dir: Path):
        ...
        download_path = temp_storage_dir / "nested" / "dir" / "file.txt"
        local_storage_provider.download(storage_path, download_path)

        assert download_path.exists()
        assert download_path.parent.exists()

    def test_download_nonexistent_file_raises_error(self, local_storage_provider: LocalStorageProvider, temp_storage_dir: Path):
        ...
        with pytest.raises(FileNotFoundError, match="File not found in storage"):
            local_storage_provider.download("nonexistent.txt", download_path)

    def test_download_overwrites_existing_file(self, local_storage_provider: LocalStorageProvider, sample_text_file: Path, temp_storage_dir: Path):
        ...
        download_path = temp_storage_dir / "existing.txt"
    ...
    def test_download_with_path_object(self, local_storage_provider: LocalStorageProvider, sample_text_file: Path, temp_storage_dir: Path):
...
class TestLocalStorageProviderDelete:
    ...
    def test_delete_file_success(self, local_storage_provider: LocalStorageProvider, sample_text_file: Path):
        """Test successful file deletion."""
        storage_path = "test.txt"
        local_storage_provider.upload(sample_text_file, storage_path)

        full_path = local_storage_provider.base_directory / storage_path
        assert full_path.exists()

        local_storage_provider.delete(storage_path)
        assert not full_path.exists()

    def test_delete_nonexistent_file_raises_error(self, local_storage_provider: LocalStorageProvider):
        """Test delete of nonexistent file raises FileNotFoundError."""
        with pytest.raises(FileNotFoundError, match="File not found"):
            local_storage_provider.delete("nonexistent.txt")

    def test_delete_file_in_subdirectory(self, local_storage_provider: LocalStorageProvider, sample_text_file: Path):
    ...
    def test_delete_does_not_remove_parent_directories(self, local_storage_provider: LocalStorageProvider, sample_text_file: Path):
        ...
        parent_dir = local_storage_provider.base_directory / "subdir"
...
class TestLocalStorageProviderListFiles:
    ...
    def test_list_files_empty_storage(self, local_storage_provider: LocalStorageProvider):
        """Test list_files on empty storage."""
        files = local_storage_provider.list_files()
        assert files == []

    def test_list_files_returns_all_files(self, local_storage_provider: LocalStorageProvider, sample_text_file: Path):
        """Test list_files returns all files."""
        # Upload multiple files
        local_storage_provider.upload(sample_text_file, "file1.txt")
        local_storage_provider.upload(sample_text_file, "file2.txt")
        local_storage_provider.upload(sample_text_file, "subdir/file3.txt")
    ...
    def test_list_files_with_prefix_filter(self, local_storage_provider: LocalStorageProvider, sample_text_file: Path):
        """Test list_files with prefix filter."""
        # Upload files with different prefixes
        local_storage_provider.upload(sample_text_file, "images/photo1.jpg")
        local_storage_provider.upload(sample_text_file, "images/photo2.jpg")
        local_storage_provider.upload(sample_text_file, "documents/doc1.pdf")

        # List only images
        image_files = local_storage_provider.list_files(prefix="images/")
        assert len(image_files) == 2
        assert all(f.startswith("images/") for f in image_files)

    def test_list_files_returns_sorted_results(self, local_storage_provider: LocalStorageProvider, sample_text_file: Path):
        """Test list_files returns sorted results."""
        # Upload files in random order
        local_storage_provider.upload(sample_text_file, "c.txt")
        local_storage_provider.upload(sample_text_file, "a.txt")
        local_storage_provider.upload(sample_text_file, "b.txt")

        files = local_storage_provider.list_files()
        assert files == ["a.txt", "b.txt", "c.txt"]

    def test_list_files_uses_forward_slashes(self, local_storage_provider: LocalStorageProvider, sample_text_file: Path):
        """Test list_files uses forward slashes regardless of OS."""
        local_storage_provider.upload(sample_text_file, "subdir/nested/file.txt")

        files = local_storage_provider.list_files()
        assert "subdir/nested/file.txt" in files
        assert "\\" not in files[0]  # No backslashes

    def test_list_files_excludes_directories(self, local_storage_provider: LocalStorageProvider, sample_text_file: Path):
        """Test list_files only returns files, not directories."""
        local_storage_provider.upload(sample_text_file, "dir1/file.txt")

        # Create empty directory
        empty_dir = local_storage_provider.base_directory / "empty_dir"
        empty_dir.mkdir()
...
class TestLocalStorageProviderPresignedURL:
    ...
    def test_generate_presigned_url_success(self, local_storage_provider: LocalStorageProvider, sample_text_file: Path):
        """Test successful presigned URL generation."""
        storage_path = "test.txt"
        local_storage_provider.upload(sample_text_file, storage_path)

        url = local_storage_provider.generate_presigned_url(storage_path)
    ...
    def test_generate_presigned_url_with_custom_expiry(self, local_storage_provider: LocalStorageProvider, sample_text_file: Path):
        """Test presigned URL with custom expiration time."""
        storage_path = "test.txt"
        local_storage_provider.upload(sample_text_file, storage_path)

        expires_in = 7200  # 2 hours
        url = local_storage_provider.generate_presigned_url(storage_path, expires_in=expires_in)
    ...
    def test_generate_presigned_url_nonexistent_file_raises_error(self, local_storage_provider: LocalStorageProvider):
        """Test presigned URL for nonexistent file raises FileNotFoundError."""
        with pytest.raises(FileNotFoundError, match="File not found"):
            local_storage_provider.generate_presigned_url("nonexistent.txt")

    def test_generate_presigned_url_encodes_special_characters(self, local_storage_provider: LocalStorageProvider, sample_text_file: Path):
...
class TestLocalStorageProviderSecurity:
    ...
    def test_path_traversal_prevention_upload(self, local_storage_provider: LocalStorageProvider, sample_text_file: Path):
        ...
        with pytest.raises(IOError, match="Upload failed.*directory traversal detected"):
            local_storage_provider.upload(sample_text_file, "../../../etc/passwd")

    def test_path_traversal_prevention_download(self, local_storage_provider: LocalStorageProvider, temp_storage_dir: Path):
        ...
        with pytest.raises(IOError, match="Download failed.*directory traversal detected"):
            local_storage_provider.download("../../../etc/passwd", download_path)

    def test_path_traversal_prevention_delete(self, local_storage_provider: LocalStorageProvider):
        ...
        with pytest.raises(IOError, match="Deletion failed.*directory traversal detected"):
            local_storage_provider.delete("../../../etc/passwd")

    def test_path_traversal_prevention_list(self, local_storage_provider: LocalStorageProvider):
        """Test list_files with traversal prefix still works safely."""
        # This should not raise error but should return empty list
        files = local_storage_provider.list_files(prefix="../")
        assert files == []

    def test_absolute_path_rejection(self, local_storage_provider: LocalStorageProvider, sample_text_file: Path):
        """Test rejection of absolute paths."""
        with pytest.raises(IOError, match="Upload failed.*directory traversal detected"):
            local_storage_provider.upload(sample_text_file, "/absolute/path/file.txt")


class TestLocalStorageProviderErrorHandling:
    """Test LocalStorageProvider error handling."""

    def test_upload_io_error_handling(self, local_storage_provider: LocalStorageProvider, sample_text_file: Path):
        ...
        readonly_dir = local_storage_provider.base_directory / "readonly"
        readonly_dir.mkdir()
    ...
    def test_download_io_error_propagation(self, local_storage_provider: LocalStorageProvider, temp_storage_dir: Path):
        """Test download propagates IO errors."""
        with pytest.raises(FileNotFoundError):
            local_storage_provider.download("nonexistent.txt", temp_storage_dir / "output.txt")

    def test_delete_io_error_propagation(self, local_storage_provider: LocalStorageProvider):
...
class TestStorageWorkflows:
    ...
    def test_upload_download_delete_workflow(self, local_storage_provider: LocalStorageProvider, sample_text_file: Path, temp_storage_dir: Path):
        ...
        # Upload
        local_storage_provider.upload(sample_text_file, storage_path)
        assert (local_storage_provider.base_directory / storage_path).exists()
        ...
        # Delete
        local_storage_provider.delete(storage_path)
        assert not (local_storage_provider.base_directory / storage_path).exists()

    def test_multiple_file_operations(self, local_storage_provider: LocalStorageProvider, sample_text_file: Path):
        """Test operations with multiple files."""
        files = ["file1.txt", "file2.txt", "file3.txt"]

        # Upload multiple files
        for file_path in files:
            local_storage_provider.upload(sample_text_file, file_path)

        # List all files
        listed_files = local_storage_provider.list_files()
        assert len(listed_files) == 3
        assert set(listed_files) == set(files)

        # Delete one file
        local_storage_provider.delete("file2.txt")

        # Verify remaining files
        remaining_files = local_storage_provider.list_files()
        assert len(remaining_files) == 2
        assert "file2.txt" not in remaining_files

    def test_nested_directory_operations(self, local_storage_provider: LocalStorageProvider, sample_text_file: Path):
        ...
        # Verify all uploaded
        all_files = local_storage_provider.list_files()
        assert len(all_files) == 3

        # List with prefix
        level2_files = local_storage_provider.list_files(prefix="level1/level2/")
        assert len(level2_files) == 2

        # Delete nested file
        local_storage_provider.delete("level1/level2/level3/file3.txt")
