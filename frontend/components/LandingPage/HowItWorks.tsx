...
import { Search, Sparkles, Download, ArrowRight, CheckCircle2 } from 'lucide-react'
...
const StepCard = memo(({ num, title, desc, icon: Icon, features, color }: Step) => {
	return (
		<div className="group relative flex-1">
			...
			<div className={`absolute inset-0 bg-gradient-to-br ${color} rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 blur-xl`} />
			
			{/* Card content */}
			<div className="relative bg-background/95 backdrop-blur-sm border border-border rounded-2xl p-6 md:p-8 h-full transition-all duration-300 hover:shadow-2xl hover:scale-[1.02] hover:border-foreground/20">
				
				{/* Icon */}
				<div className="w-14 h-14 rounded-xl bg-gradient-to-br from-foreground/10 to-foreground/5 flex items-center justify-center mb-4 group-hover:scale-110 transition-transform duration-300">
		...
		<section id="how-it-works" className="relative border-b border-border py-20 md:py-32 overflow-hidden">
			...
			<div className="container relative mx-auto px-4 lg:px-8">
				...
				<div className="text-center mb-16 md:mb-20">
					<div className="inline-block px-4 py-1.5 bg-foreground/5 border border-foreground/10 rounded-full text-sm font-medium mb-4">
						Simple Process
					</div>
					<h2 className="text-4xl md:text-5xl lg:text-6xl font-bold mb-4 bg-gradient-to-r from-foreground via-foreground/90 to-foreground/80 bg-clip-text text-transparent">
				...
				{/* Steps grid */}
				<div className="grid grid-cols-1 lg:grid-cols-[1fr_auto_1fr_auto_1fr] gap-8 lg:gap-0 max-w-7xl mx-auto items-start">
					{steps.map((step, i) => (
